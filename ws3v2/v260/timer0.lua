function write_oled() -- Write Display
  ...
  if (page == 0) then
    ...
    t2 = t2/100
    display_page0(t1, t2)
    if (ready == true and mqtt_cnt < 5) then 
      mqtt:publish("ws3/t_out", string.format("%.1f", t1), 0, 0)
      mqtt:publish("ws3/t_in", string.format("%.1f", t2), 0, 0)
      mqtt_cnt = mqtt_cnt + 2
    end
  end
  ...
  if (page == 1) then
    ...
    display_page1(h, p)
    if (ready == true and mqtt_cnt < 5) then 
      mqtt:publish("ws3/humi", string.format("%.1f", h), 0, 0)
      mqtt:publish("ws3/pressure", string.format("%.1f", p), 0, 0)
      mqtt_cnt = mqtt_cnt + 2
    end
  end
  ...
  if (page == 2) then
    ...
    display_page2(t3)
    if (ready == true and mqtt_cnt < 5) then 
      mqtt:publish("ws3/t_heat", string.format("%.1f", t3), 0, 0)
      mqtt_cnt = mqtt_cnt + 1
    end
  end
