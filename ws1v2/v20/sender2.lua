-- Weather station #1 By (R)soft 1.5.2018 v2.0
...
function sendData()
  ...
  -- close enduser_setup portal after 3 minutes
  ...
  conn:on("connection",
    function(conn) print("Connected")
      conn:send('GET /update?key=Z0ZFCZCZYJZBURGEP' .. -- your API KEY!!!
  ...
  conn:on("connection",
    function(conn) print("Connected")
      conn:send('GET /update?key=Z0ZFCZCNYZZABREZ' .. -- API KEY!!!
      '&field1=' .. string.format("%.1f", t1) ..
      '&field2=' .. string.format("%.1f", t2) ..
      '&field3=' .. string.format("%.1f", p) ..
      '&field4=' .. string.format("%.1f", h) ..
...
function write_OLED() -- Write Display
  ...
  if (page==0) then page = 1
    ...
    if (h<0) then h = 0 end
    p = bme280.baro() -- read from BME280 or BMP180
    p = p/1330.322365
    disp:firstPage()
    repeat
